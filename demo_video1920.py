...
if __name__ == "__main__":
    ...
    cap = cv2.VideoCapture("/home/cl/Ultra-Fast-Lane-Detection/test_video.MP4")
    rval,frame = cap.read()
    frame = frame[288:878,0:1640,:]
    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
    vout = cv2.VideoWriter(str(111)+'.avi', fourcc , 30.0, (int(frame.shape[1]),int(frame.shape[0])))
            ...
            if np.sum(out_j[:, i] != 0) > 2:
                for k in range(out_j.shape[0]):
                    ...
                    if out_j[k, i] > 0:
                        ...
                        if abs((int(out_j[first, i] * col_sample_w * frame.shape[1] / 800) - 1)-640)<200:
                            ppp = (int(out_j[k, i] * col_sample_w * frame.shape[1] / 800) - 1, int(frame.shape[0] - k * 20) - 1)
